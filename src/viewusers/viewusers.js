...
import { dialogButtonsStyle, dialogStyle, iconStyle, saveIconStyle } from "./viewusersStyles.js";
...
function ViewUsers(props) {
    ...
    try {
      const response = await axios.put(link, data, {
        headers: {
          //'Content-Type': 'application/json',
          Authorization: localStorage.getItem('token'),
          'X-User-Role': Cookies.get('userRole')
        }
      }).then((response) => {
        if (response.data.data === null)
          setMessage(response.data.message)
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const filteredUsers = data.filter((user) => user.email !== localStorage.getItem('email'));
        ...
        <KFHMenu >
          ...
          <Grid container xs={12} sm={12} md={12} lg={12} xl={12} justifyContent='center' alignItems='center'> 
            ...
            <Grid container xs={12} sm={12} md={6} lg={6} xl={6} sx={ButtonGrid}>
              <Button variant="contained" sx={ButtonStyle1} onClick={()=> navigate('/signupusers')} >Add A User</Button>
            </Grid>
            {users &&
              users.length > 0 &&
              users.filter((user) => {
